...
"""
function rdaam_define_constraints(model,density,set_dev,set_val,L,times,T0,T,U238,Th232,U238_V,Th232_V;
...
for j in eachindex(U238)
    if (j<n_data_constraints) || (n_data_constraints==1)
      my_constr = @NLconstraint(model, [i = 1:1],(rdaam_forward_diffusion(density,n_iter,c0,c1,c2,c3,alpha,rmr0,U238_V[i],Th232_V[i],U238[i],Th232[i],E_L,L,T0,kappa,D0_L2,E_trap,omega_rho,psi_rho,eta_q,L_dist,times...,T...)-set_val[i]+set_dev[i])^2<=(set_val[i]*0.005)^2)


    elseif (j==n_data_constraints)
      my_constr2 = @NLconstraint(model, [i = 2:2],(rdaam_forward_diffusion(density,n_iter,c0,c1,c2,c3,alpha,rmr0,U238_V[i],Th232_V[i],U238[i],Th232[i],E_L,L,T0,kappa,D0_L2,E_trap,omega_rho,psi_rho,eta_q,L_dist,times...,T...)-set_val[i]+set_dev[i])==(set_val[i]*0.0))
    end
end
